...
export default function MobileBottomNav() {
    ...
    const showNav = ['/dashboard', '/dashboard/services', '/dashboard/community', '/dashboard/settings'].includes(pathname);
    ...
    return (
        <nav className="fixed bottom-0 w-full bg-background-light dark:bg-background-dark border-t border-slate-100 dark:border-white/5 px-6 py-3 flex justify-between items-center z-50 pb-safe">
            ...
            <div className="relative -top-6">
                <button className="h-14 w-14 rounded-full bg-primary shadow-lg shadow-primary/40 flex items-center justify-center text-white transform active:scale-95 transition-transform">
            ...
            <Link href="/dashboard/settings" className={`flex flex-col items-center gap-1 w-12 transition-colors ${isActive('/dashboard/settings') ? 'text-primary' : 'text-slate-400 dark:text-slate-500 hover:text-primary'}`}>
