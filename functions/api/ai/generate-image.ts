...
export const onRequestPost = async (context: FunctionContext<Env>): Promise<Response> => {
    try {
        ...
        const { story, context: storyContext, isQuad, keywords } = await context.request.json() as {
            story?: string;
            context?: string;
            isQuad?: boolean;
            keywords?: string[];
      ...
      **Story**: "${story}"
      **Context**: "${storyContext || 'General Memory'}"
    ...
    `;
        ...
        if (error.status === 400 && error.message?.includes('safety system')) {
            return Response.json({ error: 'SAFETY_FILTER_TRIGGERED' }, { status: 400 });
        }
