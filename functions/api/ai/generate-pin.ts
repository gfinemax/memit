...
export const onRequestPost = async (context: FunctionContext<Env>): Promise<Response> => {
    try {
        ...
        const { theme, pinLength, existingDigits, existingWords } = await context.request.json() as {
            theme?: string;
            pinLength?: number;
        ...
        if (!theme || !pinLength) {
            return Response.json({ error: 'theme and pinLength are required' }, { status: 400 });
        }
...
JSON 형식:
{
  ...
  "story": "전체 스토리를 한 문장으로 만들어주세요"
        ...
        return Response.json({ error: 'Failed to generate PIN' }, { status: 500 });
    }
};
